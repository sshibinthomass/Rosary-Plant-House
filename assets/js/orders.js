      ...
      <tr>
        ...
        <td class="p-0">
          <div class="d-flex justify-content-center mt-2">
            <button type="button" id="${order.id}" value="${order.id}" class="btn btn-primary" data-toggle="modal" data-target="#order${order.id}">
            ${order.id}-Order
            </button>
          </div>
          ...
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header p-0 ml-3 pt-2">
                <h5 class="modal-title" id="exampleModalLabel">Delivery Charges</h5>
...
function filterPreload()
{
  const products = new Products();
  const ui = new UI();
  products.getProducts().then((products) => {   ui.filter(products,"0 1","Top Down Both","High Medium Low NA Waiting InProgress","V_Hgh High Medium Low V_Loow NotStarted");});
}
filterPreload()
//Nav-bar
{
...
function showAllOrders()
{
  ...
  const products = new Products();
  const ui = new UI();
  const prod=products.getProducts().then((products));
  ...
  orders.getOrders().then((orders) => {   ui.allOrders(orders);});
